...
The `0.0.1` release gate uses the [first-release adversarial matrix](security/adversarial/first-release-manifest.json) and the owner-only [first-release physical lifecycle workflow](.github/workflows/first-release-lifecycle.yml). It requires passing, fresh evidence for macOS Hardened and Linux npm Convenience on both architectures, binds every result to the candidate source and artifact digests, and requires the owner's signed approval before the meta-package can be staged or the release finalized. The first meta workflow run attaches the signed policy, CLI candidate, SBOM, and manifest to the draft release; the run after physical testing reuses and verifies those exact artifacts. The physical test covers installation, Agent enrollment, MCP use, concurrent use, repair, reinstall, purge, and uninstall. Upgrade, downgrade, and forward rollback require a prior functional release and remain in the [later-version lifecycle gate](.github/workflows/platform-lifecycle.yml).
