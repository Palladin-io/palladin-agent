...
function safeEnvironment(home, prefix) {
  ...
  environment.PATH = process.platform === 'win32'
    ...
    : `${join(prefix, 'bin')}:/usr/local/bin:/usr/bin:/bin:${dirname(process.execPath)}`;
...
function installNativeExtra(target, phase, env, work) {
  ...
  if (target.os === 'windows') {
    ...
    bounded('sudo', ['apt-get', 'install', '--yes', extra.path], { env, timeout: 600_000 });
  } else {
    bounded('sudo', ['dnf', 'install', '--assumeyes', extra.path], { env, timeout: 600_000 });
  }
}
...
function npmUninstall(phase, prefix, env) {
  ...
  const platformName = `@palladin/${platform.filename.replace(/^palladin-|-\d+\.\d+\.\d+\.tgz$/g, '')}`;
  bounded(npmExecutable(), ['uninstall', '--global', '--ignore-scripts', '--prefix', prefix, '@palladin/agent', platformName], { env, timeout: 300_000 });
}
...
function versionCheck(prefix, env, expected) {
  const capture = runCli(prefix, env, ['--version']);
  const text = capture.stdout.toString('utf8').trim(); capture.stdout.fill(0); capture.stderr.fill(0);
  if (text !== `palladin ${expected}` && text !== expected) fail('installed launcher version is invalid');
}
...
export async function runPhysicalTarget({ contract, manifest: manifestInput = loadManifest() }) {
  ...
  try {
    installNativeExtra(target, baseline, env, root); npmInstall(baseline, prefix, env); versionCheck(prefix, env, baseline.version);
    ...
    installNativeExtra(target, candidate, env, root); npmInstall(candidate, prefix, env); versionCheck(prefix, env, candidate.version);
    ...
    const platform = candidate.artifacts.find((item) => item.role === 'platform-npm');
    const platformDirectory = join(globalRoot(prefix, env), '@palladin', platform.filename.replace(/^palladin-|-\d+\.\d+\.\d+\.tgz$/g, ''));
    if (!existsSync(platformDirectory)) fail('installed platform package is unavailable');
    const hidden = `${platformDirectory}.missing`;
    renameSync(platformDirectory, hidden);
    try {
      ...
      if (!existsSync(platformDirectory)) fail('npm reinstall did not repair the missing runtime');
    } finally {
      if (!existsSync(platformDirectory) && existsSync(hidden)) renameSync(hidden, platformDirectory);
