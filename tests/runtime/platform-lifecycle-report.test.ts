...
const grants = 'f'.repeat(64);
const now = new Date('2026-07-15T10:20:00.000Z');
const runId = '123456';
const runAttempt = 2;
...
export function lifecycleFixture() {
  ...
  const steps = manifest.steps.map((definition: { id: string; order: number }, index: number) => {
    ...
    return {
      ...
      evidenceRef: `github-actions://runs/${runId}/attempts/${runAttempt}/targets/TARGET/steps/${definition.id}`,
